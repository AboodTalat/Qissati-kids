...
`app/admin-manifest.webmanifest/route.js` makes `/admin` the installed app's start URL and uses the delivered full Qissati mark at 192px and 512px. It is an explicit route linked only by the admin layout: a root metadata manifest is discovered by the public landing pages too, which would advertise an admin-only PWA to visitors and crawlers. `AdminApp` registers `public/qissati-sw.js` from every dashboard view, so the browser can offer installation before the operator visits `حسابي`. The worker has **no fetch handler and no cache at all**. Installability and background push are useful; a durable offline copy of orders, child details or photos would contradict the dashboard's `no-store` boundary.
